...
async function main() {
    ...
    try {
        ...
        console.log(
            `Minimum shares (${Number(SLIPPAGE_BPS) / 100}% slippage): ${ethers.formatUnits(minimumShares, vaultDecimals)}\n`
        );
        ...
        const depositTx = await teller.deposit(assetAddress, depositAmount, minimumShares);
