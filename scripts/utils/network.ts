 ...
 */
export function getCurrentNetworkNamespace(): FlareNetworkNamespace {
 ...
 */
export async function initializeFtsoV2Contract(currentNetworkNamespace: FlareNetworkNamespace): Promise<Contract> {
    const provider = hre.ethers.provider;

    const iFlareContractRegistryAddress = FlareContractRegistryAddress;

    const iFlareContractRegistryArtifact = currentNetworkNamespace.interfaceAbis.IFlareContractRegistry;
