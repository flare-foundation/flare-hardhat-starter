...
async function main() {
    ...
    let _totalMintedReservedUBA = BigInt(0);
    ...
    for (const agent of agents) {
        ...
        _totalMintedReservedUBA += BigInt(info.mintedUBA) + BigInt(info.reservedUBA);
        ...
        const isAgentActiveOrLiquidation =
            Number(info.status) === AGENT_STATUS_NORMAL || Number(info.status) === AGENT_STATUS_LIQUIDATION;
    ...
    if (mintingCap > 0n) {
        ...
        console.log("\n--- Minting Cap Usage ---");
        console.log(
            `Used:      ${Number(usedAmount) / Math.pow(10, Number(assetDecimals))} FXRP (${usagePercentage.toFixed(2)}%)`
        );
        console.log(
            `Remaining: ${Number(remainingAmount) / Math.pow(10, Number(assetDecimals))} FXRP (${remainingPercentage.toFixed(2)}%)`
        );
