...
async function validateMint(vault: any, account: string, sharesAmount: bigint) {
...
async function calculateAssetsNeeded(vault: any, sharesAmount: bigint) {
    const assetsNeeded = await vault.previewMint(sharesAmount);
...
async function executeMint(vault: any, sharesAmount: bigint, account: string) {
    const mintTx = await vault.mint(sharesAmount, account, { from: account });
...
async function main() {
    ...
    await approveTokens(assetToken, vault, assetsNeeded, account);
