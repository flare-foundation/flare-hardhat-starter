...
contract SimpleFtsoExample {
     ...
     */
    function getCurrentTokenPriceWithDecimals(string memory feedName)
        public
        view
        returns (uint256 _price, int8 _decimals)
    {
     ...
     */
    function getTokenPriceInUSDWei(string memory feedName)
        public
        view
        returns (uint256 _priceInWei, uint256 _finalizedTimestamp)
    {
        // WARNING: This is a test contract, do not use it in production
        TestFtsoV2Interface ftsoV2 = ContractRegistry.getTestFtsoV2();
        // Query the FTSO system
        (_priceInWei, _finalizedTimestamp) =
            ftsoV2.getFeedByIdInWei(ContractRegistry.getFtsoFeedIdConverter().getFeedId(1, feedName));
    }

    // Checks if token1/token2 price ratio is higher than numerator/denominator
    // Might overflow
    function isPriceRatioHigherThan(string memory token1, string memory token2, uint256 numerator, uint256 denominator)
        public
        view
        returns (uint256 _price1, uint256 _price2, bool _is_higher)
    {
        // WARNING: This is a test contract, do not use it in production
        TestFtsoV2Interface ftsoV2 = ContractRegistry.getTestFtsoV2();
