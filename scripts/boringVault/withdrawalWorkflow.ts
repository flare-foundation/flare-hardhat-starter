...
async function main() {
    ...
    try {
        ...
        console.log(
            `Minimum assets (${Number(SLIPPAGE_BPS) / 100}% slippage): ${ethers.formatUnits(minimumAssets, assetDecimals)}\n`
        );
        ...
        const withdrawTx = await teller.bulkWithdraw(assetAddress, withdrawShares, minimumAssets, signer.address);
