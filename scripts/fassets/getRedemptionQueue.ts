...
async function main() {
  ...
  // Deploy the contract
  const fAssetsRedemptionQueueReader =
    (await FAssetsRedemptionQueueReader.deploy(
      ASSET_MANAGER_ADDRESS,
    )) as FAssetsRedemptionQueueReaderInstance;
  await fAssetsRedemptionQueueReader.waitForDeployment();
  const fAssetsRedemptionQueueReaderAddress =
    await fAssetsRedemptionQueueReader.getAddress();
  console.log(
    "FAssetsRedemptionQueueReader deployed to:",
    fAssetsRedemptionQueueReaderAddress,
  );

  const redemptionQueue = await fAssetsRedemptionQueueReader.getRedemptionQueue(
    0,
    20,
  );
